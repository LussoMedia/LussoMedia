...
// Reusable Field Guide OG template (Part 33) — logo, "FIELD GUIDE" label,
// category, title. Static/generated-per-guide via next/og at request time;
// no dynamic-image service was introduced.
export const alt = 'Lusso Media Field Guide';
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const category = guide ? getCategory(guide.category) : undefined;
  const logoData = readFileSync(join(process.cwd(), 'public/images/logo.png'));
  const logoBase64 = `data:image/png;base64,${logoData.toString('base64')}`;
  const title = guide?.title ?? 'Lusso Media Field Guide';
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', fontSize: 15, fontWeight: 700, letterSpacing: '0.14em', color: '#5A5B5C' }}>
            FIELD GUIDE
          </div>
        </div>
        ...
        >
          Lusso Media &mdash; The Local Dominance System
        </div>
